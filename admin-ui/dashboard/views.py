...
import bcrypt
import json
from .models import Domain, User, Alias, QuotaUsage, SystemSetting
...
def users_list(request):
    users = User.objects.select_related('domain').prefetch_related('quotausage').order_by('-created_at')
...
def user_create(request):
    if request.method == 'POST':
        try:
            ...
            quota = int(request.POST.get('quota', 1)) * 1024 * 1024 * 1024  # Convert GB to bytes
    ...
    return render(request, 'dashboard/user_form.html', {'domains': domains, 'action': 'Create'})
...
def user_edit(request, user_id):
    ...
    if request.method == 'POST':
        try:
            ...
            user.quota = int(request.POST.get('quota', 1)) * 1024 * 1024 * 1024  # Convert GB to bytes
...
def domain_edit(request, domain_id):
    ...
    return render(request, 'dashboard/domain_form.html', {'domain': domain, 'action': 'Edit'})
...
# Add the format_bytes filter to templates
